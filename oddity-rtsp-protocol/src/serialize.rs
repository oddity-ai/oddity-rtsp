...
mod tests {
  ...
  fn serialize_options_request() {
    let request_bytes = Bytes::from(b"OPTIONS rtsp://example.com/media.mp4 RTSP/1.0\r\n\
CSeq: 1\r\n\
...
".as_slice());
  ...
  fn serialize_options_request_any() {
    let request_bytes = Bytes::from(b"OPTIONS * RTSP/1.0\r\n\
CSeq: 1\r\n\
\r\n\
".as_slice());
  ...
  fn serialize_options_response() {
    let response_bytes = Bytes::from(b"RTSP/1.0 200 OK\r\n\
CSeq: 1\r\n\
...
".as_slice());
    ...
    let response = Response::new(
      ...
      BTreeMap::from([
        ("CSeq".to_string(), "1".to_string()),
        ("Public".to_string(), "DESCRIBE, SETUP, TEARDOWN, PLAY, PAUSE".to_string()),
      ]),
      None);
  ...
  fn serialize_options_response_error() {
    let response_bytes = Bytes::from(b"RTSP/1.0 404 Stream Not Found\r\n\
CSeq: 1\r\n\
\r\n\
".as_slice());
  ...
  fn serialize_describe_request() {
    let request_bytes = Bytes::from(b"DESCRIBE rtsp://example.com/media.mp4 RTSP/1.0\r\n\
CSeq: 2\r\n\
\r\n\
".as_slice());
  ...
  fn serialize_describe_request_v2() {
    let request_bytes = Bytes::from(b"DESCRIBE rtsp://example.com/media.mp4 RTSP/2.0\r\n\
CSeq: 2\r\n\
\r\n\
".as_slice());
  ...
  fn serialize_describe_request_version_unknown_errors() {
    ...
    let mut request_serialized = BytesMut::new();
    assert!(if let Err(Error::VersionUnknown) = request.serialize(&mut request_serialized) { true } else { false})
  }
...
a=StreamName:string;\"hinted audio track\"".as_slice());
    ...
    let response = Response::new(
      ...
      BTreeMap::from([
        ("CSeq".to_string(), "2".to_string()),
        ("Content-Base".to_string(), "rtsp://example.com/media.mp4".to_string()),
        ("Content-Length".to_string(), "443".to_string()),
        ("Content-Type".to_string(), "application/sdp".to_string()),
      ]),
      Some(Bytes::from(b"m=video 0 RTP/AVP 96
a=control:streamid=0
a=range:npt=0-7.741000
a=length:npt=7.741000
a=rtpmap:96 MP4V-ES/5544
...
a=StreamName:string;\"hinted audio track\"".as_slice())));
  ...
  fn serialize_play_request() {
    let request_bytes = Bytes::from(b"PLAY rtsp://example.com/stream/0 RTSP/1.0\r\n\
CSeq: 1\r\n\
Content-Length: 16\r\n\
Session: 1234abcd\r\n\
\r\n\
